...
2. 空間排版對齊與「非典型單據」推理（特別針對置物櫃、車票、自動販賣機收據）：
   ...
   - ⚠️ **欄位與雜訊隔離（極重要）**：單據上常出現大量與金額無關的 3 或 4 位數（例如：置物櫃號碼 Box No.、終端機編號 Terminal No.、條碼編號）。當提取金額（`unit_price` / `total_foreign_amount`）時，必須尋找緊鄰 `¥`、`\`、`金額`、`領収`、`利用料`、`小計`、`合計` 等關鍵字「同一行（y軸相近）」或「正下方（x軸相近）」的數字。
