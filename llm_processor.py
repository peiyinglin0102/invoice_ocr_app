...
3. 🧮 數學自我檢算與對帳規範（嚴禁做帳）：
   ...
   - **檢查形近字錯誤與排版**：如果加總不符，請高度警惕是否發生了 OCR 形近字誤判，特別是排版與貨幣標記干擾：
     ...
     * **「内 \」或「内 ¥」與首位數的混淆（一般化規則）**：
       - 當品項單價或金額前包含 `内` 或貨幣符號 `\`、`¥` 時，OCR 經常將 these 符號與價格的第一個數字黏在一起，辨識成多了一位數（例如將 `¥33` 識為 `133` 或 `733`，將 `¥88` 識為 `188`）。
...
5. 雜訊與座標過濾：
   - 存入 `original_name` 時，請務必「乾淨移除空間座標前綴與多餘條碼數字」（鍵入如將 `[y:230, x:40] 3101パペットスンスン` 轉為純淨的原始商品名 `パペットスンスン`）。
...
class LLMProcessor:
    """
    Gemini 3.5 Flash LLM 處理器 (純 REST API 版，含自動避開限流重試機制)
    """
    ...
    def process_ocr_texts(self, ocr_texts: List[str]) -> Dict[str, Any]:
        ...
        logger.info(f"Sending {len(ocr_texts)} OCR text blocks to {self.model_name}...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 💡 大招：如果遇到伺服器過載 (429 高需求/限流 或 503 服務不可用)，自動延時重試
                if response.status_code in (429, 503) or "experiencing high demand" in response.text:
                    if attempt < max_retries - 1:
                        logger.warning(f"Gemini API 門檻限流 (HTTP {response.status_code})。正在進行第 {attempt+1} 次延時重試...")
                ...
                if attempt < max_retries - 1:
                    ...
                    continue
                raise ValueError(f"ERR-004: Gemini API 呼叫失敗 — {str(e)}") from e